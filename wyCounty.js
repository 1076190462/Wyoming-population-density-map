...
d3.csv("wyPop.csv", function(error,data) {
...
//  ]);
  ...
  d3.json("wy-counties.json", function(error, wy) {
    
    /* Bind data from caPop2014.csv file and ca.json file */
    var counties_json = topojson.feature(wy, wy.objects.counties);
    for (var i = 0; i < data.length; i++) {
      ...
      */
      for (var j = 0; j < counties_json.features.length; j++) {
        var wy_county = counties_json.features[j].properties.name;
        if (county_name == wy_county) {
